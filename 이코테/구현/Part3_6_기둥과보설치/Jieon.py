...
def check(n,now_frame):
    for i in range(n):
        for j in range(n):
            if 1 in now_frame[i][j]: #교차점에 보가 있을 경우, 한쪽끝이 기둥(0)이거나, 양쪽끝이 보(1)
                if (0 in now_frame[i-1][j]) or (0 in now_frame[i-1][j+1]) or ((1 in now_frame[i][j-1]) and (1 in now_frame[i][j+1])):
                    continue
                else: 
                    return False
            if 0 in now_frame[i][j]: #교차점에 기둥이 있을 경우, 바닥 위(n,0)이거나 한쪽 끝이 보(1) 이거나 기둥(0)위
...
def solution(n, build_frame):
    wall=[[[[2]] for _ in range(n+1)] for __ in range(n+1)] #(0~n)
    result=[]
